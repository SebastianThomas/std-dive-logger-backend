 ...
 * at stage time since get-or-create by serial number is idempotent and harmless even if the
 * staged import is later discarded), source-derived {@code gasConsumption}/{@code configuration}
 * (not user-overridable - UDDF computes these from the file itself), plus the remaining metadata
 * not already covered by the cheap "guess" columns on {@code PendingImportEntity} (notes,
 * visibility, named buddies).
 *
 ...
 * doesn't override the dive number, commit attaches the profile to the existing whole-numbered
 * dive instead of creating a new one.
 */
