 ...
 * "Gas must consist of 100%") used to surface to the client as a bare, generic message from
 * {@code ResponseEntityExceptionHandler}'s own default handling, with the actual reason only ever
 * visible in the server logs.
 *
 * <p>Jackson's own wrapping exception - and Spring's {@link HttpMessageNotReadableException}
 * wrapping that in turn - both preserve the failing constructor's message verbatim behind a
 * literal {@code "problem: "} marker (see {@code GasJsonDeserializationTest} for exactly what this
 * looks like). Extracting that gives every record's compact-constructor validation across the
 * whole app - not just {@code Gas} - a real, specific message on the client, instead of only in
 * the logs.
 *
 * <p>This can't be a plain {@code @ExceptionHandler} default method like the other advice traits
 * in this package: {@code ResponseEntityExceptionHandler} (which {@code ExceptionHandling}
 * extends) already claims {@code HttpMessageNotReadableException} on its own {@code final
