...
public interface DiveTagRepository extends JpaRepository<DiveTagEntity, Long> {
    ...
    @Modifying
    @Query("DELETE FROM DiveTagEntity t WHERE t.dive.id = :diveId AND t.manual = false AND t.dismissed = false")
    void deleteActiveAutoTagsByDiveId(long diveId);

    /**
     * Returns the tag-definition IDs that are already "covered" for a given dive —
     * i.e. rows that are either manual or dismissed and must not be overwritten.
     */
